package work.iruby.leetcode.medium;//给你一个下标从 0 开始的正整数数组 candiesCount ，其中 candiesCount[i] 表示你拥有的第 i 类糖果的数目。同时给你一个二维数
...
class Solution1744 {
    static class Test {
        public static void main(String[] args) {
            Solution1744 test = new Solution1744();
            long start = System.currentTimeMillis();
            System.out.println(Arrays.toString(test.canEat(new int[]{7, 4, 5, 3, 8},
    ...
    public boolean[] canEat(int[] candiesCount, int[][] queries) {
        ...
        for (int i = 0; i < m; i++) {
            ...
            int minDay = sum[favoriteType] / dailyCap + 1;
            int maxDay = sum[favoriteType + 1];
