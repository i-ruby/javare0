package work.iruby.leetcode.medium;//给你一个数组 orders，表示客户在餐厅中完成的订单，确切地说， orders[i]=[customerNamei,tableNumberi,foodIt
...
class Solution1418 {
    static class Test {
        public static void main(String[] args) {
            Solution1418 test = new Solution1418();
    ...
    public List<List<String>> displayTable(List<List<String>> orders) {
        ...
        Map<Integer, Map<String, Integer>> displayOrder = new HashMap<>();
        for (List<String> order : orders) {
            Integer tableId = Integer.parseInt(order.get(1));
            String foodName = order.get(2);

            Map<String, Integer> map = displayOrder.getOrDefault(tableId, new HashMap<>());

            map.put(foodName, map.getOrDefault(foodName, 0) + 1);
            displayOrder.put(tableId, map);
            foodSet.add(foodName);
        }
        List<String> foodList = new ArrayList<>(foodSet);
        Collections.sort(foodList);

        List<String> displayTitle = new ArrayList<>();
        displayTitle.add("Table");
        displayTitle.addAll(foodList);

        List<Integer> tableId = new ArrayList<>(displayOrder.keySet());
        Collections.sort(tableId);

        List<List<String>> display = new ArrayList<>();
        display.add(displayTitle);
        for (Integer id : tableId) {
            List<String> list = new ArrayList<>();
            list.add(id + "");

            Map<String, Integer> map = displayOrder.get(id);
            for (String foodName : foodList) {
                list.add(map.getOrDefault(foodName, 0) + "");
            }
            display.add(list);
        }
